...
| **Q-3** | Quality | Medium | Partial | Continued token-bridge modularization by extracting inline tests into `token_bridge/tests.rs` and expression/operand parsing helpers into `token_bridge/expr_helpers.rs`; `token_bridge.rs` reduced to 1,758 lines; additional parser/bridge decomposition pending |
