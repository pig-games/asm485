...
| **Q-3** | Quality | Medium | Closed | Completed token-bridge modularization by extracting inline tests (`token_bridge/tests.rs`), directive parser cluster (`token_bridge/directives.rs`), expression/operand helpers (`token_bridge/expr_helpers.rs`), and parser-VM execution helpers (`token_bridge/parser_vm.rs`); `token_bridge.rs` reduced to 825 lines |
...
| **T-5** | Coverage | Medium | Closed | Added `proptest`-based property harness for binary codec decode determinism on arbitrary byte inputs, complementing seeded mutation-fuzz regression coverage |
...
| **Q-8** | Perf | Low | Closed | Completed follow-up lowercase-allocation cleanup in runtime encode paths: resolver/capability lookups now use borrowed fast-path lookup with fallback normalization, avoiding repeated `to_ascii_lowercase()` allocations on already-normalized IDs |
